...
def this_scripts():
    ...
    for i in range(len(df)):
        ...
        if 'NIFTY' in df['symbol'][i][:10] and 'NFO' in df['exch_seg'][i]:
...
def working_day_calculation(value):
    ...
    expiry_BANKNIFTY=expiry_list('BANKNIFTY')


    expiry_1=option_symbol('BANKNIFTY',expiry_BANKNIFTY[0])
    expiry_2=option_symbol('BANKNIFTY',expiry_BANKNIFTY[1])
    # expiry_2=option_symbol('BANKNIFTY',)
